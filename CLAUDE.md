...
conda run -n hw5-ontology python shacl/validate.py  # SHACL 結構驗證（選用，§15）
```

**推理 vs 驗證的分工**：`reasoning.py` 用 OWL 等價類公理「推理」class 成員（GraspableObject、BallConcealingCup）；`validate.py` 用 SHACL shapes（`shacl/shapes.ttl`）「驗證」圖是否滿足 §15 結構約束（每個 PhysicalObject 有 objectLabel、每個任務目標有 taskRole + affordance）。驗證以 rdfs inference 執行，結果 **Conforms: True**，報告存 `shacl/shacl_validation_report.txt`。所有 SHACL 檔案集中於選用的 `shacl/` 資料夾，使核心倉庫結構維持對齊 PDF §16.1。
...
完整執行日誌（含四階段推理逐步過程）會同時寫入 `log/reasoning.log`（UTF-8）。
此 log 內含本機絕對路徑且每次重跑會覆寫，故列入 `.gitignore` 不提交（SHACL 的 `shacl/validate.log` 同樣不提交）。
...
- [x] SHACL 驗證（PDF §15）— 集中於選用的 `shacl/` 資料夾（`shapes.ttl` + `validate.py` + `shacl_validation_report.txt`，Conforms: True）
